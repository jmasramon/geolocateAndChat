...
chatRoom = io.of('/chat').on('connection', function(socket) {
    ...
    socket.on('nick', function(info) {
        console.log('onNick: User ' + info.nickName + ' connected')
        ...
        console.log('onNick: New array of users: ' + JSON.stringify(users, null, 4));
    ...
    socket.on('disconnect', function() {
        ...
        console.log("onDisconnect: Initial users = " + JSON.stringify(users, null, 4) + " For socket = " + socket.id);
        console.log("onDisconnectUser num " + users.indexOf(user) + " disconnecting." + " For socket = " + socket.id);
        users.splice(
        users.indexOf(user), 1); // Elimina l'usu de la llista d'usus connectats
        console.log("onDisconnect Final users= " + JSON.stringify(users, null, 4) + " For socket = " + socket.id);
    ...
    socket.on('positionChange', function(info) {
        console.log('onPositionChange: Datos recividos = ' + info.nickName + ', ' + info.lat + ', ' + info.lng);
        console.log("onPositionChange: Initial users = " + JSON.stringify(users, null, 4));
        ...
        console.log("onPositionChange: Info received = " + JSON.stringify(info, null, 4));
        var movingUser = grep(users, function(e) {
            return e.nickName == info.nickName;
        }, false);

        console.log('onPositionChange: Moving user = ' + JSON.stringify(movingUser[0], null, 4));
        console.log('onPositionChange: Index of movingUser = ' + users.indexOf(movingUser[0]));

        users.splice(users.indexOf(movingUser[0]), 1); // Elimina l'usu de la llista d'usus connectats
        console.log('onPositionChange: Array of users after splice = ' + JSON.stringify(users, null, 4));
        ...
        console.log('onPositionChange: Moved user reintroduced. New array of users = ' + JSON.stringify(users, null, 4));
    });
